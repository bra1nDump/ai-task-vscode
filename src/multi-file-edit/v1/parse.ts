...
export function parsePartialMultiFileEdit(xml: string): LlmGeneratedPatchXmlV1 {
  ...
  const changesToMultipleFiles: FileChange[] = fileChangeOutputs.map(
    (fileChangeOutput) => {
  ...
  return { changes: changesToMultipleFiles, isStreamFinalizedUnused: false }
}
