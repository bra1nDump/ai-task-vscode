/*
For historical purposes, I'm keeping the prompts that helped generate the file.

I'm trying to explore the api for vscode.workspace.OnDidChangeTextDocumentEvent
And how it interacts with the edit events. I'm also trying to explore various ways of performing edits
Create a test suite similar to parse test in style that showcases how various 
events are handled.
...
suite('vscode.workspace.OnDidChangeTextDocumentEvent', () => {
  ...
  test('Deleting a range of lines', async () => {
    ...
    assert.ok(editor)
    await editor.edit((editBuilder) => {
      // Notice how we are technically deleting up until the first character on the third line.
      // I guess rangers are not closed on the right side.
  ...
  // The edits are reported starting with the parts in the file that come later in the file.
  // This is done so if we replayed the edits in the same order, we would get the same result.
  // Otherwise we would need to keep track of changing ranges (because they're pushed down or pushed up) as we apply edits.
