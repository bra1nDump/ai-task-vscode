...
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  const boardNames = [];
  for (const adjective of randomFunnyAdjectives) {
    for (const noun of randomFunnyNouns) {
      boardNames.push(`board-${adjective}-${noun}`);
    }
  }

  for (const name of boardNames) {
    console.time(`SHARD ${name}`);
    const boardCountDeleted = await cleanupStaleMiroBoardsJobShardedByName(
      name
    );
    console.timeEnd(`SHARD ${name}`);
    console.log(`SHARD ${name} COMPLETED: ${boardCountDeleted} boards deleted`);
  }
}

/**
 * One of the shortcomings is deleting boards will cause the next link
 * to skip some of the boards due to shift after deletion. We don't really care about this
 * all we care about is reducing the number of boards to a reasonable number.
 * Since this is a daily job it will eventually clean up all the boards.
 */
export async function cleanupStaleMiroBoardsJobShardedByName(
  // Work around the ten thousand limit by sharding by name
  nameQuery: string
): Promise<number> {
  ...
  // Offset was useful when we were not using next link
  // Increasing offset to start getting older boards from the get go
  let offset = 1_000;
  let failures = 0;
  const limit = 50;
  ...
  function getNextLink(
    ...
    // I'm not entirely sure last opened will not mess things up, but I'm hoping to do this in combination with offset
    // to avoid cleaning boards that are actively being interacted with, no matter when edited
    return `https://api.miro.com/v2/boards?limit=${limit}&offset=${offset}&query=${nameQuery}&sort=last_opened`;
  }

  let nextLink: string | undefined = getNextLink(limit, offset, nameQuery);
  while (nextLink && failures < 2) {
    try {
      ...
      // @bread replace the deletionCount with a set of board identifiers that were deleted. Print out the entire set at the end. Only modify the following for loop and the variables and printing outside of it.
      let deletionCount = 0;
      for (const board of responseData.data) {
        ...
        // console.log(
        //   `Board ${board.name} is stale ${lastModifiedDate} and was deleted.`
        // );
      }
      totalDeletions += deletionCount;

      console.log(
        `Batch ${++batch}: Deleted ${deletionCount} / ${
          responseData.size
        } boards`
      );

      // Alternatively we could have been creating our own next link, but this is easier
    ...
    // Half a minute will give us roughly 50% of the rate limit
    await delay(30_000);
  }
...
```md
<task>
The task is to replace the deletionCount with a set of board identifiers that were deleted and print out the entire set at the end. This involves modifying the for loop in the cleanupStaleMiroBoardsJobShardedByName function, along with the associated variables and printing operations outside of it. The function is located in the src/miro.ts file.
</task>
<change>
<path>src/miro.ts</path>
<range-to-replace>
...
<description>
Context: code block inside a while loop
1: Define a set to hold board identifiers of deleted boards
2: In the for loop, if a board is to be deleted, add its id to the set
3: After the for loop, add the size of the set to totalDeletions
4: Print out the set of deleted board identifiers
</description>
<replacement>
      ...
      totalDeletions += deletedBoards.size;

      console.log(
        `Batch ${++batch}: Deleted ${deletedBoards.size} / ${
          responseData.size
        } boards`,
        'Deleted board IDs:',
        Array.from(deletedBoards)
      );
</replacement>
</change>