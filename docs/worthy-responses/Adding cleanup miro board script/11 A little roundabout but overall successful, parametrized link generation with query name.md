...
// @bread parametrize the link generation with nameQuery
...
```md
<task>
The task is to parameterize the link generation function in the cleanupStaleMiroBoardsJobShardedByName function in the context of the cleanup stale Miro boards job. The specific function to modify is getNextLink, which generates the API request link for the Miro boards. The change should allow the query parameter for the board name to be dynamic, based on a provided nameQuery argument. The stale board cleanup job is part of an effort to manage board creation and deletion to stay within API rate limits.
</task>
...
1: Initialize nextLink by calling getNextLink with limit, offset and nameQuery parameters
</description>
<replacement>
...
<description>
Context: function
Input: limit: maximum number of boards to retrieve, offset: offset of the first item in the collection to return, nameQuery: name of the board to query
Output: API request link for Miro boards
1: Return a string that concatenates the base URL, limit, offset, and nameQuery parameters to create a complete API request link
</description>
<replacement>
  function getNextLink(
    limit: number,
    ...
    return `https://api.miro.com/v2/boards?limit=${limit}&offset=${offset}&query=${nameQuery}`;
  }
</replacement>
</change>