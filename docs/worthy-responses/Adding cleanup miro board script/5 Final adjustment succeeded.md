...
@crust  
Your task is to update the function below based on the feedback in the next lines.
...
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  ...
  let nextLink:
    | string
    | undefined = `https://api.miro.com/v2/boards?limit=${limit}&offset=${offset}&sort=last_opened`;
...
```md
<task>
Refactor the cleanupStaleMiroBoardsJob function based on the following instructions:
1. Create the "next" link manually instead of using the one provided in the API response.
2. Comment out the code that deletes the boards, as the application is currently running in a driver mode.
3. Count the number of boards that would be deleted in each batch and print this number, also printing the total batch number.
</task>
...
Input: none
Output: Promise<void>
1: Define a delay function that returns a promise which resolves after a given duration in milliseconds
2: Initialize offset to 0 and failures to 0
3: Define limit as 50
4: Define someDaysAgo as the date three days ago from now
5: Define serviceUserId as the ID of the Miro service user from the configuration
6: Define nextLink as the initial link to fetch the first batch of boards
7: While nextLink is not undefined and failures is less than 2
8:   Try to fetch the boards from the next link
9:   For each board in the fetched data
10:     If the board's last modified date is older than someDaysAgo and the board was last modified by the service user
11:       Comment out the code that deletes the board
12:     End If
13:   End For
14:   If the number of boards fetched is less than the limit, set nextLink to undefined
15:   Otherwise, manually construct the next link by increasing the offset by the limit
16:   Delay for 60 seconds
17:   Reset failures to 0
18:   If an error occurred during the fetch or deletion operations
19:     Log the error
20:     Increment failures by 1
21:     Delay for 60 seconds
22:   End Try
23: End While
24: If failures is greater than or equal to 2, log an error message indicating that two subsequent failures occurred during the board querying process
25: Log a message indicating that stale Miro boards would have been deleted
</description>
<replacement>
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  const delay = (ms: number) =>
  ...
  while (nextLink && failures < 2) {
    try {
      ...
      console.log(`Batch ${offset / limit + 1}: Would have deleted ${deletionCount} boards`);

      if (boards.data.length < limit) {
        nextLink = undefined;
      } else {
        offset += limit;
        nextLink = `https://api.miro.com/v2/boards?limit=${limit}&offset=${offset}&sort=last_opened`;
      }
  ...
  console.log("Would have deleted stale Miro boards");
}
</replacement>
</change>