...
nameQuery is me trying to work around the problem with too many boards.
 */
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  // @crust your task is to create queries for all the permutations of the board names (reference name creation in the beginning of the file). Then sequentially run cleanupStaleMiroBoardsJobShardedByName for each of the queries. Also replace the hardcoded name in the query in the next function
}
...
export async function cleanupStaleMiroBoardsJobShardedByName(
  ...
  function getNextLink(
    ...
    return `https://api.miro.com/v2/boards?limit=${limit}&offset=${offset}&query=adorable-ninja`;
  }
  ...
  while (nextLink && failures < 2) {
    try {
      ...
      console.log(
        `Batch ${++batch}: Would have deleted ${deletionCount} / ${
          responseData.size
        } boards`
      );
...
```md
<task>
The task is to implement a job that cleans up stale Miro boards. The job should be designed to run daily and should delete boards that were created more than 5 days ago and have not been updated since. The job should be careful not to exceed the rate limits set by the Miro API, which are 200 requests per minute for creating and deleting boards. To avoid hitting the rate limits, the cleanup job should query boards in batches of 50, delaying by one minute between each batch. This should use about 25% of the rate limit. The task involves creating queries for all possible permutations of the board names and running the cleanup job for each of these queries. The cleanup job should also handle any errors that may occur during the process.
</task>
...
Input: none
Output: void
1: Generate all possible permutations of board names
2: Sequentially run cleanupStaleMiroBoardsJobShardedByName for each query
<replacement>
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  const allNames = getAllPermutationsOfBoardNames();
  for (const name of allNames) {
    await cleanupStaleMiroBoardsJobShardedByName(name);
  }
}
</replacement>
</change>
...
<description>
Context: function
Input: nameQuery: query name to be used
Output: number of deletions
1: Replace hardcoded name in the query with the input nameQuery
<replacement>
export async function cleanupStaleMiroBoardsJobShardedByName(
  nameQuery: string
): Promise<number> {
