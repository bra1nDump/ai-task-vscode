...
```md
<task>
The task is to update the function `cleanupStaleMiroBoardsJob()`. The updates should include the following:

1. Instead of using the `nextLink` from the API response, it should be created manually.

2. The deletion of boards should be commented out because the function is running in "driver" mode.

3. The function should count the number of boards that would be deleted in each batch and print it out, along with the total batch number.
</task>

<change>
<path>src/miro.ts</path>
<range-to-replace>
async function cleanupStaleMiroBoardsJob(): Promise<void> {
  <truncated/>
  console.log("Deleted stale Miro boards");
}
</range-to-replace>
<description>
Context: function
1: Define delay function to wait for specified amount of milliseconds
2: Initialize offset, failures count, limit of boards in one batch and date someDaysAgo to 3 days ago
3: Define serviceUserId from config
4: Define nextLink with limit and offset
5: While nextLink exists and there were less than 2 failures
6:   Try to get boards from nextLink
7:   For each board if it was modified more than someDaysAgo and was modified by service user
8:     Comment out the line that deletes board
9:   If there were less boards in the batch than limit, clear nextLink
10:  Wait for 60 seconds
11:  Reset failures count
12:  If there was an error increment failures count and wait for 60 seconds
13: If there were 2 subsequent failures, log error message
14: Log message about deleting stale Miro boards
</description>
<replacement>
async function cleanupStaleMiroBoardsJob(): Promise<void> {
  const delay = (ms: number) =>
  ...
  let totalBoardsToDeleteCount = 0;
    ...
    try {
      ...
      for (const board of boards.data) {
        ...
        if (
          ...
          boardsToDeleteCount++;
          // Comment out deletion of boards
          // await axiosWrapper.delete(
          //   `https://api.miro.com/v2/boards/${board.id}`
          // );
        }
      ...
      totalBoardsToDeleteCount += boardsToDeleteCount;
      console.log(`Batch ${offset / limit + 1} - Boards to delete: ${boardsToDeleteCount}`);
  ...
  console.log(`Total boards to delete: ${totalBoardsToDeleteCount}`);
}
</replacement>
</change>
