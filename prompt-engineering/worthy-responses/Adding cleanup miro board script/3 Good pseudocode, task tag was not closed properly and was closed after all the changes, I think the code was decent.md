...
```md
<task>
The task is to modify the cleanupStaleMiroBoardsJob function in the miro.ts file. The function should be rewritten to address the following points:

1. The function should not use the non-existent "lastOpenedAt" field. Instead, only use the fields from the returned board object in the Miro API response.

2. The function should introduce a delay between the batches of API requests to ensure that the rate limit is not reached.

3. A board should be considered active if it was created less than 5 days ago, or if it was ever modified by a user different from the service user account. The ID of the service user account should be retrieved from an environment variable named SERVICE_USER_ID.
...
Context: function
Output: Promise<void>
1: Initialize constants for batch size, delay between batches, and staleness threshold
2: Initialize date from staleness threshold days ago
3: Initialize offset for initial query
4: While there's a next link
5:   Query boards with current offset
6:   For each board in the result
7:     If the board was created more than staleness threshold days ago and was not modified by a user other than service user
8:       Delete the board
9:   Wait for delay between batches
10:  Update offset for next query
</description>
<replacement>
import { config } from "./environment";
...
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  try {
    ...
    staleDate.setDate(staleDate.getDate() - stalenessThreshold);

    let offset = 0;

    while (true) {
      const response = await axiosWrapper.get(
        `https://api.miro.com/v2/boards?limit=${batchSize}&offset=${offset}`
      );
      const boards = response.data;
      ...
      for (const board of boards.data) {
        ...
        const modifiedByUserId = board.modifiedBy.id;

        if (createdDate < staleDate && modifiedByUserId !== config.miro.serviceUserId) {
