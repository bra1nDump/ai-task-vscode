...
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  ...
  let batch = 0;
  // @crust create a helper function to create the next link, also change the sort to last_created. This is your task. Use the helper to create the original link and then to create subsequent links within the loop. Only update affected code regions.
  let nextLink:
    | string
  ...
  while (nextLink && failures < 2) {
    try {
      ...
      for (const board of responseData.data) {
        ...
        console.log(
          `Board ${board.name} is stale and would have been deleted.`
        );
      }
...
```md
<task>
The task is to create a helper function that will generate the next link for fetching boards from the Miro API, and to modify the way the initial link and subsequent links are created within the cleanupStaleMiroBoardsJob function. The helper function should replace the hardcoded link string that includes the limit, offset and sort parameters. The sort parameter should be changed to 'last_created'. Only the code regions relevant to the creation of the next link should be affected.
</task>
...
1: Define a helper function to generate the next link for fetching boards
2: Use the helper function to generate the initial link
</description>
<replacement>
function getNextLink(limit: number, offset: number, sort: string = 'last_created'): string {
...
1: Update the offset
2: Use the helper function to generate the next link
</description>
<replacement>
offset += responseData.data.length;
