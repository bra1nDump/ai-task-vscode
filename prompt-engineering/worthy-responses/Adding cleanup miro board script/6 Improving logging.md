  ...
  while (nextLink && failures < 2) {
    try {
      ...
      for (const board of boards.data) {
        ...
        // @crust Your task is to split this if statement by first skipping the boards that are too new, then checking if the board was modified by a real user and not the service user. Improved logging for both cases.
        if (
          ...
          console.log(
            `Board ${board.name} was last modified by ${board.modifiedBy.name} ${lastModifiedDate} and is not stale`
          );
        }
      }
      console.log(
        `Batch ~${Math.floor(
          offset / limit + 1
        )}: Would have deleted ${deletionCount} boards`
      );

      // For some reason the apia has returned forty nine boards. Lets half just in case
      if (boards.data.length < limit / 2) {
        nextLink = undefined;
      } else {
        offset += boards.data.length;
...
```md
<task>
The task involves refactoring the Miro board cleanup function in the application. The current function iterates through Miro boards and deletes those that are older than a certain date and were last modified by a specific user. The task involves splitting a complex conditional statement that checks both of these conditions. 

The new implementation should first exclude boards that are too new, and then check if the last modification was made by a specific user. The logging in the function should also be improved to provide clearer information about the boards being processed. 

The Miro board cleanup function is located in the 'cleanupStaleMiroBoardsJob' function in the 'src/miro.ts' file.
...
<description>
Context: for loop inside a function
1: For each board, check if its modification date is more recent than 'someDaysAgo'
2: If it is, skip to the next board
3: If it's not, check if it was modified by the service user
4: If it was, increment the deletion count and delete the board
5: If it wasn't, log that the board is not stale and was modified by a different user
6: After the loop, log the number of boards that would have been deleted
</description>
<replacement>
let deletionCount = 0;
for (const board of boards.data) {
  ...
  if (lastModifiedDate >= someDaysAgo) {
    console.log(`Board ${board.name} is too new and will not be deleted.`);
    continue;
  }

  if (board.modifiedBy.id !== serviceUserId) {
    console.log(
      `Board ${board.name} was last modified by ${board.modifiedBy.name} on ${lastModifiedDate} and will not be deleted as it was not modified by the service user.`
    );
    continue;
  }
  ...
  console.log(`Board ${board.name} is stale and would have been deleted.`);
}

console.log(
  `Batch ~${Math.floor(offset / limit + 1)}: Would have deleted ${deletionCount} boards`
);
</replacement>
</change>