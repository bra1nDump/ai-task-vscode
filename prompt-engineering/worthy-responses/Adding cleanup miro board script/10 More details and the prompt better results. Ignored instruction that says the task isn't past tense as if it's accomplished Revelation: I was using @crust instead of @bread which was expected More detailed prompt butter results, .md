...
```md
<task>
Your task is to create queries for all the permutations of the board names. These names are constructed from the two arrays, `randomFunnyAdjectives` and `randomFunnyNouns`. You need to create permutations manually from these two arrays. Then, you should sequentially run `cleanupStaleMiroBoardsJobShardedByName` for each of the queries. 
</task>
...
<description>
Context: function
1: Generate all the permutations of the board names manually from the two arrays provided
2: Sequentially run cleanupStaleMiroBoardsJobShardedByName for each of the queries
</description>
<replacement>
export async function cleanupStaleMiroBoardsJob(): Promise<void> {
  ...
  for (const adjective of randomFunnyAdjectives) {
    for (const noun of randomFunnyNouns) {
      namePermutations.push(`board-${adjective}-${noun}`);
    }
  }
  
  for (const nameQuery of namePermutations) {
    await cleanupStaleMiroBoardsJobShardedByName(nameQuery);
  }
}
</replacement>
</change>