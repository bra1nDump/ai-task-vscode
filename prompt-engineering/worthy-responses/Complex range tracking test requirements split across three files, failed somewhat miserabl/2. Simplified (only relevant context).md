How to automatically get those contexts?
- Keep the entire file where the generation happens
- Resolve type of an intermediate variable: lineRangeToBeReplaced: LineRange
- 
...
`
  ...
  test('When appending to the range keeps extending its end', async function () {
    // write a test that will demonstrate the issue I mentioned within the apply resolve change file. Used the same edits from above, but now diff the current content within the range with the replacement and and apply insert edits instead of replacements. This test is expected to fail which is fine. 
    // In your response do not print any explanations simply print out the code that should go in this body. 
  })
})
```
